...
int DhcpClass::beginWithDHCP(uint8_t *mac, const char* hostname, unsigned long timeout, unsigned long responseTimeout)  // add hostname argument (JSB 2023-03-10)
{
	_dhcpLeaseTime=0;
	_dhcpT1=0;
	_dhcpT2=0;
	...
	// set the hostname JSB 2023-03-10
	if (hostname)
	{
...
{
	uint8_t BUFFER_SIZE = 32 + MAX_HOST_NAME_LENGTH;  // JSB 2023-03-10
	uint8_t buffer[BUFFER_SIZE];  // JSB 2023-03-10                        
	memset(buffer, 0, BUFFER_SIZE);  // JSB 2023-03-10
	...
	memset(buffer, 0, BUFFER_SIZE); // clear local buffer (JSB 2023-03-10)
	...
	// OPT - host name (JSB 2023-03-10)
	uint8_t hostname_length = strlen(_dhcpHostname);
	buffer[16] = hostName;
	buffer[17] = hostname_length;
	strlcpy((char*)&(buffer[18]), _dhcpHostname, MAX_HOST_NAME_LENGTH + 1);

	//put data in W5100 transmit buffer
	_dhcpUdpSocket.write(buffer, 18 + hostname_length);  // JSB 2023-03-10
